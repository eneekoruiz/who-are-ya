...
import {calculateTimeUntilMidnight} from "./Rows.js";
...
let showStats = function() {
    ...
    const modalHTML = headless(statsHTML);
    document.body.appendChild(stringToHTML(modalHTML));

    const interval = setInterval(() => {
        const nextPlayerElement = document.getElementById('nextPlayer');
        if (nextPlayerElement) {
            nextPlayerElement.textContent = calculateTimeUntilMidnight();
        }
    }, 1000);
